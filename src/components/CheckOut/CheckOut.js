...
//Toast Notification
const noInput = () => toast.warn("Please input your Credit Card Info", {
    position: "top-center",
    autoClose: 1000,
...
const invalidName = () => toast.warn("Please check your input name", {
    position: "top-center",
    autoClose: 1000,
    hideProgressBar: false,
    closeOnClick: true,
...
const invalidCardNumber = () => toast.warn("Invalid Card Numbers ", {
    position: "top-center",
    autoClose: 1000,
    hideProgressBar: false,
    closeOnClick: true,
...
const invalidCVC = () => toast.warn("Invalid CVC", {
    position: "top-center",
    autoClose: 1000,
    hideProgressBar: false,
    closeOnClick: true,
...
const CheckOut = ({ cart }) => {
    ...
    var input_credit_card = function(input){
        ...
        card_name.addEventListener('keyup', function(){
            let name_holder = document.querySelector('.name-holder');
            if(card_name.value == ''){
    ...
    const handleSubmit = () => {
        ...
        const hashedCard = CryptoJS.AES.encrypt(JSON.stringify(creditCard), "sha256").toString();
    ...
    useEffect(() => {
        ...
        card_bg.style.backgroundImage = 'url(https://raw.githubusercontent.com/muhammederdem/credit-card-form/master/src/assets/images/' + Math.floor(Math.random() * 25 + 1) + '.jpeg';
        input_credit_card(credit_card);

        get(child(ref(db), `creditCardInfo/${uid}`)).then((snapshot) => {
            if (snapshot.exists()) {
                const bytes = CryptoJS.AES.decrypt(snapshot.val().creditCard, 'sha256');
                const decrypt = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));
                
                document.querySelector('#credit-card').value = decrypt;
                document.querySelector('#card-name').value = snapshot.val().cardHolder;
                document.querySelector('#exp-month').value = snapshot.val().expMonth;
                document.querySelector('#exp-year').value = snapshot.val().expYear;

                document.querySelector('#card-number').innerHTML = document.querySelector('#credit-card').value;
                document.querySelector('.name-holder').innerHTML = document.querySelector('#card-name').value;
        ...
        pay.addEventListener('click', () => { 
            ...
            const regex_creditCard = /^(5[1-5][0-9]{2}) ([0-9]{4}) ([0-9]{4}) ([0-9]{4})$/ 
            ...
            const expMonth = document.querySelector('#exp-month').value;
            const expYear = document.querySelector('#exp-year').value;
            if(creditCard == "" || cardHolder == "" || expMonth == "" || expYear == "" || cvc == "") {
                noInput();
                check = false;
            }
        ...
        close_btn.addEventListener('click', () => { popup.classList.remove('active'); });
        ...
        <div className="main">
            ...
            <section className='popup'>
                ...
                <div className="model-box">
                    <BsPatchQuestion/>
                    <h2>Question</h2>
                    <h3>Do you want to store your credit card for the next payment again?</h3>
                    <div className="buttons">
                        <button className="keep-btn" onClick={handleSubmit}>Keep</button>
                        <button className="close-btn" onClick={goNext}>No!</button>
                    </div>
                </div>
            </section>
            <div className="container">
                <div className="outer">
                    <header className="head-item">
                        <div className="logo">
                            <a href="#"><strong>.zZ</strong>-Commerce</a>
                        </div>
                        ...
                        <div className="left">
                            <div className="field">
                                ...
                                <div className="card-holder">
                                    ...
                                    <div className="input">
                                        <input type="text" id='card-name' placeholder="Ex. John Wick " autoComplete='off' required/>
                                    </div>
                                </div>
                                <div className="card-cvc">
                                    ...
                                    <div className="input">
                                        <input type="password" id="cvc" placeholder="Ex. 232" maxLength={3} required/> {/* use onInput*/}
                                    </div>
                                </div>
                                <div className="card-expiration">
                                    ...
                                    <div className="input_exp">
                                        <select className='select month' id="exp-month" required>
                                        </select>
                                        <strong> / </strong>
                                        <select className='select year' id="exp-year">
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div className="right">
                            <div className="card-virtual">
                                ...
                                <p className="highlight">
                                    <span className="last-digit" id="card-number">Card Number</span>
                            ...
                            <div className="receipt">
                                <ul>
                                    ...
                                    <li>
                                        <span>Product</span>
                                        <a href="/cart" className="products">Go back to cart</a>
                                    </li>
                                    <li>
                                        <span>Vat (7%)</span>
                                        <span className="vat">{(parseFloat((sumPrice+"").slice(1))*30)*7/100}</span>
                                    </li>
                                </ul>
                                <div className="total">
                                    <p className="price">
                                        <strong id="price">{(parseFloat((sumPrice+"").slice(1))*30) + ((parseFloat((sumPrice+"").slice(1))*30)*7/100)} </strong>
                                        THB
                                    </p>
